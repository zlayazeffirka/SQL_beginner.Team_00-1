...
min_total_cost as (
    select
        min(total_cost + (select cost from routes where point1 = last_town and point2 = 'A')) as mtc
    from
        tour
    where
        town_count = 4
...
from
	tour
join 
	min_total_cost on min_total_cost.mtc=tour.total_cost+(select cost from routes where point1=last_town and point2='A')
where
	town_count=4
order by
	total_cost, tour;
